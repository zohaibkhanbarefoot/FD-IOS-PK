...
#define JMOScrollIndicatorBottomSpace 3.0f
#define JMOScrollIndicatorWidth 15.0f
...
-(void) enableCustomVerticalScrollIndicator:(JMOScrollIndicatorPosition)vPos withColor:(UIColor *)indicatorColor withIndicatorStyle:(JMOScrollIndicatorType)style
{
    ...
    {
        CGFloat x = self.frame.size.width - width - JMOScrollIndicatorBottomSpace;
        if([UIScreen mainScreen].bounds.size.height > 667.0f)
            x = self.frame.size.width + 85.0f;
        else if([UIScreen mainScreen].bounds.size.height == 667.0f)
            x = self.frame.size.width + 45.0f;
        frame = CGRectMake(x, 0.0f, JMOScrollIndicatorHeight,JMOScrollIndicatorWidth);
    }
    else {
    ...
    if (style == JMOScrollIndicatorTypeClassic) {
        ...
        viewScrollIndicator.layer.cornerRadius = 0.0f;//width/2.0f;
...
- (void)refreshVerticalScrollIndicatorWithAlpha:(CGFloat)alpha
{
    ...
    {
        CGFloat f = (pourcent * self.frame.size.height);
        f -= viewScrollIndicator.frame.size.height;
        rect.origin.y =  self.contentOffset.y + MAX(0.0f, f);
    }
